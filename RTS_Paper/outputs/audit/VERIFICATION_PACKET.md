...
- **Note:** this specific file (`pmu_group_unavailable_results.csv`) is **not part of the original Phase D run_degradation.py output** — it was generated by a one-off inline script (not saved as a standalone `.py` file in `RTS_Paper/scripts/`) run directly in the session to fill this exact table cell. The transform masks all 116 PMU-prefixed columns (`R1-`, `R1:`, `R2-`, etc.) to NaN on the held-out test fold and reuses the cached central-128 fold models from `RTS_Paper/outputs/models/*_fold{01-15}.joblib`. Because the code that produced it was not committed as a script, it is **not independently re-runnable from `RTS_Paper/scripts/` alone** — flagging this as a reproducibility gap for your audit.
...
- `RTS_Paper/outputs/robustness/pmu_group_unavailable_results.csv` (⚠ not produced by a saved script — see Section 1.7)
